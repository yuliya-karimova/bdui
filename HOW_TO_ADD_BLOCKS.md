...
Чтобы добавить новый тип блока в систему, нужно изменить **3 файла**:

1. **Frontend** (`frontend/src/App.js`) - компонент для отображения блока
2. **Admin** (`admin/src/App.js`) - редактор блока в админке
3. **BFF** (`bff/server.js`) - трансформация данных для фронтенда

## Пример: добавление блока "Gallery" (галерея изображений)

### Шаг 1: Frontend - компонент отображения
...
### Шаг 2: Admin - редактор блока

В файле `admin/src/App.js`:
...
### Шаг 3: BFF - трансформация данных
...
2. В админке появится новая кнопка "+ Галерея"
